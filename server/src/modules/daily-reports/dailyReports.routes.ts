...
router.get(
  "/mine",
  requirePermission(PERMISSIONS.SUBMIT_DAILY_REPORT),
  controller.listMineHandler,
);
router.get(
  "/kam/:kamId",
  requirePermission(
...
router.get(
  "/:date",
  requirePermission(PERMISSIONS.SUBMIT_DAILY_REPORT),
  controller.getByDateHandler,
);
router.post(
  "/",
  requirePermission(PERMISSIONS.SUBMIT_DAILY_REPORT),
  validateBody(upsertDailyReportSchema),
  controller.upsertReportHandler,
);
