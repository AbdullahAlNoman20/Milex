// admin/src/Pages/modules/sales/pages/TeamReportsPage.jsx (NEW FILE)
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100 text-sm">
            {kams.length === 0 ? (
              <tr>
                <td colSpan={3} className="p-8 text-center text-slate-400">No KAMs found.</td>
              </tr>
            ) : (
              kams.map((k) => (
                <tr key={k.id} className="hover:bg-slate-50 transition">
                  <td className="p-4 pl-6 font-bold text-slate-800">{k.name}</td>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center">
              ...
              <button type="button" onClick={() => setActiveKam(null)} className="text-slate-400 hover:text-slate-700 transition">
                <X size={18} />
              </button>
            </div>
              ...
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2 text-xs font-bold border-b-2 transition ${
                    tab === t.key ? 'border-emerald-600 text-emerald-700' : 'border-transparent text-slate-400'
                  }`}
                >
                  {t.label}
                </button>
            ...
            <div className="p-5 overflow-y-auto flex-1">
              {isDetailLoading ? (
                ...
                weeklyPlans.length === 0 ? (
                  <p className="text-sm text-slate-400 text-center py-8">No weekly plans yet.</p>
                ) : (
                  <div className="space-y-3">
                    {weeklyPlans.map((p) => (
                      <div key={p.id} className="border border-slate-200 rounded-lg p-4">
                        ...
                        {p.lmComments && (
                          <p className="text-xs text-red-600 mb-2"><strong>Feedback:</strong> {p.lmComments}</p>
                        )}
                ...
                <p className="text-sm text-slate-400 text-center py-8">No daily reports yet.</p>
              ) : (
                <div className="space-y-3">
                  {dailyReports.map((r) => (
