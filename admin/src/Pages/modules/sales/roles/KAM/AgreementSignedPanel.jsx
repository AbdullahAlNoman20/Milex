// admin/src/Pages/modules/sales/roles/KAM/AgreementSignedPanel.jsx (NEW FILE)
...
const AgreementSignedPanel = ({ customer }) => {
  ...
  const handleDirect = async () => {
    setIsSubmitting(true);
    await updateStatus(customer.id, 'ACTIVE_ACCOUNT', {}, 'AGREEMENT SIGNED — ACCOUNT ACTIVATED');
    setIsSubmitting(false);
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-emerald-600 p-6 space-y-4">
      <h3 className="font-bold text-slate-900 text-base flex items-center">
        <FileSignature size={18} className="mr-2 text-emerald-600" /> Agreement Signed
      </h3>
      <p className="text-xs text-slate-500 leading-relaxed">
        Confirm the signed agreement has been received from the customer, then choose how to
        create the account.
      </p>
      <button
        type="button"
      ...
      >
        <PackageCheck size={16} className="mr-2" /> Create Provisional Account (Docs Required)
      </button>
      <button
        type="button"
        disabled={isSubmitting}
