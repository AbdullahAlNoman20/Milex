// src/Pages/modules/operations/pages/Client/ImportRequest.jsx
...
const ImportRequest = () => {
    ...
    (idx) => {
      if (idx === 0) {
        for (const f of PARTY_ADDRESS_FIELDS) {
          if (f.required && !isRequired(pickup[f.key])) return `${f.label} is required`;
          if (f.type === 'email' && pickup[f.key] && !isValidEmail(pickup[f.key])) return `${f.label} must be a valid email`;
        }
        return null;
      }
      if (idx === 1) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      showToast('Import request submitted — pending Foreign Admin review');
      setSubmitted(true);
    } catch (err) {
      showToast(err?.message || 'Failed to submit import request', 'error');
    } finally {
      setIsSubmitting(false);
    }
  }, [validateStep, pickup, parcel, declaredValue, currency, shipmentType, packaging, services, currentUser, showToast]);

  const steps = useMemo(() => ([
    {
      title: 'Shipper Address',
      content: (
        <SectionCard title="Shipper / Pick Up Address">
          <div className="space-y-4">
            {PARTY_ADDRESS_FIELDS.map((f) => (
              <FormField key={f.key} field={f} disabled={isSubmitting} value={pickup[f.key]} onChange={(v) => setPickup((p) => ({ ...p, [f.key]: v }))} />
            ))}
          </div>
        </SectionCard>
      ),
      ...
      content: (
        <SectionCard title="Review & Submit">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <p><span className="font-bold text-slate-700">Shipper:</span> {pickup.companyName || '—'}</p>
    ...
    return (
      <div className="max-w-2xl mx-auto p-4 md:p-0">
        <SectionCard title="Request Submitted">
          <p className="text-sm text-slate-600 mb-4">Your import request has been sent for review.</p>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 p-4 md:p-0">
      <div>
        <h1 className="text-xl font-black text-slate-800">Import Request</h1>
