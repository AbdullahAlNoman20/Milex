// src/Pages/modules/operations/pages/ForeignAdmin/ImportRequestManagement.jsx
...
const ImportRequestManagement = () => {
  ...
  const load = useCallback(async () => {
    setIsLoading(true);
    try {
      setRows(await fetchRequestsByMode('IMPORT'));
    } catch {
      showToast('Failed to load import requests', 'error');
    } finally {
      setIsLoading(false);
    }
  }, [showToast]);
  ...
  if (isLoading) return <Loader label="Loading import requests..." />;

  return (
    <div className="max-w-5xl mx-auto space-y-6 p-4 md:p-0">
      <div>
        <h1 className="text-xl font-black text-slate-800">Import Request Management</h1>
        <p className="text-sm text-slate-500">Review client import requests. Approving auto-generates an AWB and shipment record.</p>
      </div>

      <SectionCard title="Import Requests">
        {rows.length === 0 ? (
          <p className="text-sm text-slate-400">No import requests submitted yet.</p>
        ) : (
                ...
                {expandedId === r.id && (
                  <div className="bg-slate-50 border-t border-slate-100 p-4 space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                      <div>
                        <p className="font-bold text-slate-500 uppercase text-[10px] mb-1">Shipper</p>
