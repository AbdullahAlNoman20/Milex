// src/Pages/modules/operations/pages/DomesticAdmin/PickupRequestManagement.jsx
// Domestic Admin schedules pickups for approved EXPORT shipments (from BD shippers).
...
const DomesticPickupRequestManagement = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setShipments(all.filter((s) => s.shipmentMode === 'EXPORT' && s.statusCode === 'BOOKED'));
    } catch {
  ...
  const handleMarkPickedUp = useCallback(
    async (awb) => {
      ...
      setBusyAwb(awb);
      try {
        await updateShipmentStatus(awb, { statusCode: 'PICKED_UP', note: `Pickup completed on ${date}` }, currentUser?.name);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 p-4 md:p-0">
      <div>
        ...
        <p className="text-sm text-slate-500">Export shipments booked and awaiting physical pickup from the shipper.</p>
      </div>
            ...
            {shipments.map((s) => (
              <div key={s.awbNumber} className="border border-slate-100 rounded-lg p-3 flex flex-wrap items-center gap-3">
                <div className="flex-1 min-w-[200px]">
                  <Link to={`/operations/documents/awb/${encodeURIComponent(s.awbNumber)}`} target="_blank" rel="noopener noreferrer" className="text-sm font-bold text-emerald-600 hover:underline">
                    {s.awbNumber}
                  </Link>
                  <p className="text-xs text-slate-500">{s.pickup?.companyName} — {s.pickup?.address}, {s.pickup?.city}</p>
...
export default DomesticPickupRequestManagement;