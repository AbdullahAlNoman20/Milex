...
import { ArrowLeft, Building, Printer } from 'lucide-react';
...
import ProvisionalAccountPanel from '../roles/LineManager/ProvisionalAccountPanel';
...
import OperationsCustomerView from '../roles/Operations/OperationsCustomerView';
import AccountsCustomerView from '../roles/Accounts/AccountsCustomerView';

const CustomerDetail = () => {
  const { barcode } = useParams();
  const navigate = useNavigate();
  const { currentUser } = useAuth();
  const { customers, isLoading, loadError, setSelectedCustomer, setPrintData } = useSales();
  ...
  const isReadOnlyDept = [ROLES.OPERATIONS, ROLES.ACCOUNTS].includes(role);

  const renderActionPanel = () => {
    if (isReadOnlyDept || customer.status === STATUS.ACTIVE) return null;
    ...
    if (
      [STATUS.APPROVED_PENDING_OFFER, STATUS.OFFER_DRAFTING].includes(customer.status) &&
    ...
    if (customer.accountProfileType === 'PROVISIONAL' && role === ROLES.LINE_MANAGER) {
      return <ProvisionalAccountPanel customer={customer} />;
    }
    return (
    ...
    <div className="max-w-[1400px] mx-auto space-y-6 animate-in fade-in duration-300 pb-12">
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        <div className="flex-1 min-w-0 space-y-6">
          {role === ROLES.OPERATIONS && <OperationsCustomerView customer={customer} />}
          {role === ROLES.ACCOUNTS && <AccountsCustomerView customer={customer} />}
